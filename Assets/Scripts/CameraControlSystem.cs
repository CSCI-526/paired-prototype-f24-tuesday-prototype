...
public class CameraControlSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float moveSpeed = Mathf.Lerp(moveSpeedMin, moveSpeedMax,
                                    (mainCamera.GetComponent<Camera>().orthographicSize - zoomMin) / (zoomMax - zoomMin));
        if (mousePos.x <= 0)
        ...
        if (mouseScroll != 0.0f)
        {
            float newCamZoom = mainCamera.GetComponent<Camera>().orthographicSize;
            newCamZoom -= mouseScroll * zoomSpeed;
            mainCamera.GetComponent<Camera>().orthographicSize = Mathf.Clamp(newCamZoom, zoomMin, zoomMax);
        }
    }
}
